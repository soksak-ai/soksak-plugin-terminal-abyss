...
function setup(bracketed = false) {
  ...
  const binding = attachComposition(input, { emit, bracketedPaste: () => bracketed, now: () => clock, lastKeydown: () => last });
  return { input, emit, binding, tick: (ms: number) => { clock += ms; }, keydown: (text: string) => { last = { text, at: clock }; } };
}
